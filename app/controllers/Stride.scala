...
trait Stride extends AuthorisedFunctions with AuthRedirects with FrontendBaseController with I18nSupport {
  ...
  private def convertRolesSetToPredicate(enrolments: Set[String]) =
  ...
  def authWithStride(actionName: String, f: AuthorisedUser => Future[Result])(
    ...
    if (appConfig.strideAuth) {
      ...
      authorised(convertRolesSetToPredicate(strideRoles).and(AuthProviders(PrivilegedApplication)))
        .retrieve(credentials and name) {
          case Some(strideCredentials) ~ Some(name) =>
            logger.debug(s"$actionName - authorised, name=$name")
            f(AuthorisedUser((name.name.toList ++ name.lastName.toList).mkString(" "), strideCredentials.providerId))
        }.recover {
