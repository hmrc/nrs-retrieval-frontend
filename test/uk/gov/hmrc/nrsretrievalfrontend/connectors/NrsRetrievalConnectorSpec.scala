...
class NrsRetrievalConnectorSpec extends UnitSpec with NrsSearchFixture with BeforeAndAfterEach:
  ...
  "submitRetrievalRequest" should {
    "make a post call to /retrieval-requests" in { (dataEventAuditType: DataEventAuditType) =>
    ...
    "make a post call to /retrieval-requests and retrieve nr-submission-id from header" in { (dataEventAuditType: DataEventAuditType) =>
      when(mockHttpClientV2.post(ArgumentMatchers.eq(submissionBundlesUrl))(using any[HeaderCarrier])).thenReturn(mockRequestBuilder)
  ...
  "statusSubmissionBundle" should {
    "make a head call to /submission-bundles" in { (dataEventAuditType: DataEventAuditType) =>
  ...
  "getSubmissionBundle" should {
    "make a get call to /submission-bundles" in { (dataEventAuditType: DataEventAuditType) =>
