...
class AppConfig @Inject() (
  ...
  private def loadConfig(key: String) = runModeConfiguration
  ...
  private def loadConfigWithDefault(key: String, default: String) =
    runModeConfiguration.getOptional[String](key).getOrElse(default)

  lazy val nrsRetrievalUrl =
    s"${servicesConfig.baseUrl("nrs-retrieval")}/nrs-retrieval"
  lazy val xApiKey: String = loadConfigWithDefault(
    s"microservice.services.nrs-retrieval.xApiKey",
    "missingKey"
  )

  lazy val isLocal: Boolean = loadConfigWithDefault(
    s"microservice.services.nrs-retrieval.isLocal",
    "false"
  ).toBoolean

  lazy val interval: FiniteDuration = loadConfig(
    s"polling.interval"
  ).toLong.millis
